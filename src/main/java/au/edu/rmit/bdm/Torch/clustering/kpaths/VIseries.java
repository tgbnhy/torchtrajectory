...
public class VIseries {
	static int[] trajectory;// the bdm
	int length;// the number of vertex inside the bdm
	 ...
	 * we can propose bound to avoid reading each bdm and compute the similarity model.
	 */
	public static double EDRDistanceJaccard(Integer[] T1, Integer[] T2) {
	 ...
	 */
	public static double EDRDistance(int[] T1, int[] T2) {
	...
	public double DTWDistance(VIseries T1, VIseries T2) {
		// double distance = SimilarityMeasure.DynamicTimeWarping(T1.bdm,
		// T2.bdm);
		return 0;
	}
}
