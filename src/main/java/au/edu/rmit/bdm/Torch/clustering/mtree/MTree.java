package au.edu.rmit.bdm.Torch.clustering.mtree;
...
import au.edu.rmit.bdm.Torch.clustering.mtree.SplitFunction.SplitResult;
import au.edu.rmit.bdm.Torch.clustering.kpaths.Bounds;
import au.edu.rmit.bdm.Torch.clustering.kpaths.ClusterPath;
import au.edu.rmit.bdm.Torch.clustering.kpaths.Process;
import au.edu.rmit.bdm.Torch.clustering.kpaths.Yinyang;
	...
	public class KMeansAssignment extends Yinyang{
			...
			while(!pendingQueue.isEmpty()){
				...
				for(DATA centerdata : node.children.keySet()) {
					...
					if(child instanceof MTree.Entry){//the data
						...
						if (!candidateofAllclusters.contains(idx)) {// if it is never contained by any list
							...
							for (int j = 0; j < k; j++) {
								...
								if (!canlist.contains(idx)) // it is not contained
									...
									dist = Yinyang.computeRealDistance(data, clustra, idx);
								}
								if (min_dist > dist) {
						...
						candiList.get(minId).add(child);//add the bdm into the list
		...
		public boolean assignNode(Node childNode, ArrayList<int[]> centoridData, int []data, double radius) {
			...
			for(int[] centroid: centoridData) {
				double upperBound = Yinyang.Intersection(centroid, data, centroid.length, data.length) + radius;
				if(upperBound<minUpperBound) {
					minUpperBound = upperBound;
					minId = i;
				}							
				childNode.bounds[i] = Yinyang.Intersection(centroid, data, centroid.length, data.length) - radius;
		...
		public double getMinimumLowerboundInNode(double [] bounds, int groupNumber) {
		...
		public void assignmentWithPrevious(ArrayList<int[]> centoridData) {
			...
			for (int group_i = 0; group_i < k; group_i++) {//check each group
				...
				{//check each center in the group
					...
					for (IndexItem child : tralist) { // check every bdm in the center to assign which integrate the group filtering and local filtering
						...
						int tralength = tra.length; // the length of bdm is read
						...
						if (!checkInvertedIndex(candidateofAllclusters, idx)) { // if it is never contained by any list, we can assign it to the cluster with minimum length
							...
							double lowerbound = getMinimumLowerboundInNode(bounds, k);	// bound from drift					
						...
						if(assignNode(childNode, centoridData, tra, child.radius)) {	// if the histogram is not equal to the original centroids, we move 
							if(centerID!=minId) {								
								...
								if(idxNeedsIn.containsKey(minId))
									...
									idxlist = new ArrayList<IndexItem>();
								idxlist.add(child);
								idxNeedsIn.put(minId, idxlist);// temporal store as we cannot add them the bdm list which will be scanned later, batch remove later
								if(idxNeedsOut.containsKey(centerID))
									idxlist = idxNeedsOut.get(centerID);
								else
									idxlist = new ArrayList<IndexItem>();
