...
import au.edu.rmit.bdm.Torch.clustering.streaming.streamEdge;
import java.io.BufferedReader;
import java.io.FileNotFoundException;
import java.io.FileReader;
import java.sql.*;
/*
 * the streaming k-path processing will be conducted based on the static k-path, when the bdm updated in a cluster, we will update the existing cluster directly, for the new coming bdm,
 * we will assign to the existing kpath data stream
 * 1) each bdm is limited in a limited time window
...
public class StreamKpath extends Yinyang{
	 ...
	 */
	public static void updateDataset(Map<Integer, int[]> dataset, Map<Integer, ArrayList<Integer>> olddata, 
			Map<Integer, ArrayList<Integer>> newdata) {
		ArrayList<Integer> newdataset = new ArrayList<>();//store the totally new bdm, and assign them to the
		if(olddata!=null)
	...
	public static boolean timetoupdateData() {
	 ...
	 */
	public static void readDataFromFile(String path) throws SQLException, InterruptedException{
